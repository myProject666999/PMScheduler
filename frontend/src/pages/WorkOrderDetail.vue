...
import { getWorkOrderDetail, dispatchWorkOrder, executeWorkOrder, reviewWorkOrder } from '@/api/workorder'
...
const dispatchForm = ref({ executorId: '', plannedDate: '' })
const executeForm = ref({ actualHours: 0, content: '', remark: '', parts: [] as any[] })
const reviewRemark = ref('')

const showDispatch = computed(() => order.value.status === 'PENDING' || order.value.status === 'DISPATCHED')
...
async function handleReview(passed: boolean) {
  try {
    await reviewWorkOrder(id, { passed, remark: reviewRemark.value })
    loadDetail()
  } catch {}
}
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <div class="font-medium text-industrial-900">{{ order.itemName }}</div>
        </div>
        <div>
          <div class="text-sm text-gray-500">执行人</div>
          <div class="font-medium text-industrial-900">{{ order.executorName || '-' }}</div>
        </div>
        <div>
          <div class="text-sm text-gray-500">计划日期</div>
          <div class="font-medium text-industrial-900">{{ order.plannedDate || '-' }}</div>
        </div>
        <div>
        ...
        <div>
          ...
          <div class="font-medium text-industrial-900">{{ order.completedAt || '-' }}</div>
        </div>
      </div>
      <div v-if="order.content" class="mt-4 pt-4 border-t">
        <div class="text-sm text-gray-500 mb-1">保养内容</div>
        <div class="text-gray-700 whitespace-pre-wrap">{{ order.content }}</div>
      </div>
    </div>
    ...
    <div v-if="showDispatch" class="bg-white rounded-lg shadow-sm p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input v-model="dispatchForm.executorId" placeholder="输入执行人ID" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-industrial-500" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">计划日期</label>
          <input v-model="dispatchForm.plannedDate" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-industrial-500" />
        </div>
      </div>
    ...
    <div v-if="showExecute" class="bg-white rounded-lg shadow-sm p-6">
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <textarea v-model="executeForm.content" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-industrial-500" />
        </div>
